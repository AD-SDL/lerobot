...
class OpenArmJacobianIK:
    ...
    def __init__(self, urdf_path: str | None = None):
        ...
        if urdf_path is None:
            # The OpenArm SDK from https://github.com/enactic/openarm, not the unrelated
            # `openarm` distribution on PyPI. It is not a lerobot dependency, so pass
            # urdf_path explicitly if you would rather not install it.
            from openarm.kinematics.models import OPENARM_URDF_PATH

            urdf_path = OPENARM_URDF_PATH

        # ikpy is used only for FK; the IK below is our own damped least squares.
        # Imported here rather than at module top so that importing this module is
        # free -- the hybrid teleoperator probes it by construction, not by import.
        import ikpy.chain
    ...
    def solve(
        ...
        for iteration in range(max_iterations):
            ...
            # Adaptive step size: grow up to 1.0 while improving, halve when stuck.
            adaptive_step = min(step_size * 1.2, 1.0) if error_norm < previous_error else step_size * 0.5

            previous_error = error_norm

            # Compute Jacobian. Capitalised to match the formula in the comment below;
            # renaming to satisfy pep8-naming would make the code harder to check against it.
            J = self._compute_jacobian(chain, current_joints)  # noqa: N806
            ...
            JJT = J @ J.T  # noqa: N806
            JJT_damped = JJT + damping * np.eye(3)  # noqa: N806
    ...
    def solve_right_arm(
        self, target_position: np.ndarray, initial_joints: np.ndarray | None = None, **kwargs
    ) -> np.ndarray:
        ...
        solution, converged, error = self.solve(self._right_chain, target_position, initial_joints, **kwargs)
    ...
    def solve_left_arm(
        ...
        solution, converged, error = self.solve(self._left_chain, target_position, initial_joints, **kwargs)
...
class OpenArmIKWrapper:
    ...
    def solve_right_arm_physical(
        ...
        urdf_solution = self.ik_solver.solve_right_arm(target_position, initial_joints=urdf_initial, **kwargs)
    ...
    def solve_left_arm_physical(
        ...
        # Solve IK in URDF frame
        urdf_solution = self.ik_solver.solve_left_arm(target_position, initial_joints=urdf_initial, **kwargs)
