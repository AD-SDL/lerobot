...
class MoveItIKSolver:
    ...
    def __init__(self, node_name='moveit_ik_solver'):
        ...
        self.left_ik_client = self.node.create_client(
            GetPositionIK,
            '/compute_ik'
        )
        
        self.right_ik_client = self.node.create_client(
            GetPositionIK,
            '/compute_ik'
        )
...
class OpenArmMoveItIKWrapper:
    ...
    def compute_fk(self, urdf_joints_rad: np.ndarray, arm: str = "right") -> np.ndarray:
        ...
        """
        if arm == "left":
            pos, _ = self._fk_solver._forward_kinematics(
                self._fk_solver._left_chain,
                urdf_joints_rad
            )
        else:
            pos, _ = self._fk_solver._forward_kinematics(
                self._fk_solver._right_chain,
                urdf_joints_rad
            )
        return pos
