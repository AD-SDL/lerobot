...
class OpenArmBiGamepadJointsTeleop(Teleoperator):
    ...
    def action_features(self) -> dict[str, type]:
        # Flat {key: type}, matching get_action() exactly -- see the note there on why
        # .vel and .torque are present and zero.
    ...
    def connect(self, calibrate: bool = True):
        ...
        # hasattr is true even when start() failed to find a pad -- and then
        # .get_name() raises on None instead of falling back to the defaults below.
    ...
    def send_feedback(self, feedback):
        if not self.initialized:
            ...
            logger.info(
                f"Initialized - Left: {self.left_joint_positions[:7]}, Right: {self.right_joint_positions[:7]}"
            )
